...
description: |
  Indonesia’s economy plunged into a recession in the third quarter of 2020, the country’s first recession since the 1998 Asian Financial Crisis. The economy shrank as household consumption and investment contracted due to the COVID-19 pandemic. Most sectors showed declines, with a few exceptions such as the information and communication sector. But there are signs of recovery.
output: 
...
The country's GDP `r ier_compare_grow(last(growth_tidy$pct_change_yoy))` by **`r last(growth_tidy$pct_change_yoy)`** percent year-on-year (yoy) in the `r ier_period(last(growth_tidy$date))`, `r ier_compare_sev(growth_latest[2], growth_latest[1])` than the previous quarter.
...
The pandemic has led the GDP to contract by **5.32** percent yoy in the second quarter of 2020 and pushed it to the first recession in two decades as it contracted by **3.49** percent yoy in the third quarter of last year.
...
The household consumption, the largest contributor to the GDP by expenditure, `r ier_compare_grow(growth_exp_hh_latest$pct_change_yoy[2])` by **`r growth_exp_hh_latest$pct_change_yoy[2]`** percent yoy in the `r ier_period(last(growth_exp_tidy$date))`, `r ier_compare_sev(growth_exp_hh_latest$pct_change_yoy[2], growth_exp_hh_latest$pct_change_yoy[1])` than the previous quarter.
...
By expenditure (percentage points)
...
The private consumption contributed **`r latest_ctg_pc`** percentage points and the investment **`r latest_ctg_inv`** percentage points to the annual GDP growth in the `r ier_period(last(ctg_exp_trf$date))`. 

The net exports and government consumption offset the contractions in other expenditure components by contributing **`r latest_ctg_nx`** and **`r latest_ctg_govt`** percentage points, respectively.
...
The manufacturing sector, the largest contributor to the GDP by sector, `r ier_compare_grow(manufacturing[2])` by **`r manufacturing[2]`** percent yoy in the `r ier_period(last(growth_sec_tidy$date))`, `r ier_compare_sev(manufacturing[2], manufacturing[1])` than the previous quarter.

<aside>

See also [employment by sector](employment.html#employment-by-sector)
